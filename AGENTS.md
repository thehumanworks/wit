...
### `formal/` — Lean 4 proofs of ADR invariants (ADR 0010)
- Lake project, core Lean only (no Mathlib), toolchain pinned in `formal/lean-toolchain`. Modules under `formal/Wit/`: `Coordinator` (FillCoordinator state machine: single-flight, budgets, storage cap, takedown), `Budget` (`Fits`: free-tier arithmetic on the deployed limits), `Integrity`, `ReadOnly`, `Streaming`, `Keys`, `Swr` (ADR 0002).
- `Wit/Generated/Constants.lean` is generated by `scripts/gen_formal_constants.mjs` from the Worker source, `wrangler.toml`, the deploy workflow, and `cloud.rs`/`ops.rs`; never edit it by hand. `Wit/Assumptions.lean` holds the only hand-written numbers (Cloudflare limits, account baseline).
- Changing a modeled limit or code shape: `bash scripts/check_formal.sh --write`, then fix the proof or `Fits` field that fails. No `sorry`/`admit`/`axiom`/`native_decide`.
...
- Before handing off, run `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `bash scripts/check_wit_search_migration.sh`, `bash scripts/check_wit_snapshot_wasm.sh`, `bash scripts/check_docs_site.sh`, `bash scripts/check_url_api_deploy_workflow.sh`, `bash scripts/check_cache_worker.sh`, and `bash scripts/check_formal.sh`. When `showcase/url-api` or `sdk/` changed, also run `(cd showcase/url-api && npm run check)`, `(cd sdk/typescript && npm run check)`, and `(cd sdk/python && python3 -m unittest discover -s tests)`.
