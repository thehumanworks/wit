...
Pass one async JavaScript function body to `code`. Use `await` with `codemode.wit.findRepositories`, `codemode.wit.refs`, `codemode.wit.open`, `codemode.wit.list`, `codemode.wit.searchCode`, `codemode.wit.read`, and `codemode.wit.context`; TypeScript syntax, imports, filesystem, network, environment, process, subprocess, shell, modules, and arbitrary host calls are unavailable. Credentials and privileged operations remain in the Rust parent.

Open once and reuse the snapshot within the parent server lifetime. Snapshots do not survive restart. Follow explicit cursors with otherwise unchanged arguments. Return one focused JSON-serializable value with repository, commit, snapshot, path, blob, and line provenance. Fixed source/time/call/page/snapshot/result budgets fail explicitly; cancellation and deadlines use stable errors. A failed worker is killed and reaped and the next invocation starts fresh. Generated source and worker diagnostic content are not persisted or logged; only capped diagnostic byte counts and truncation state are retained.
...
Experimental Code Mode instead exposes one normal MCP tool named `code`. Its input is an async JavaScript function body and its final result must be one JSON-serializable value no larger than 48 KiB. The generated methods are `codemode.wit.findRepositories`, `codemode.wit.refs`, `codemode.wit.open`, `codemode.wit.list`, `codemode.wit.searchCode`, `codemode.wit.read`, and `codemode.wit.context`. Their host results keep explicit cursors, budgets, snapshots, and provenance.
